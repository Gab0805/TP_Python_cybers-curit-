...
class ReservationDao:
    ...
    def reserver_place(cls, rsv:Reservation): 
        sql = "INSERT INTO reservations (nom, place, status, evenement) VALUES (%s, %s, %s, %s)"
        params = (rsv.nom, rsv.place, rsv.status, rsv.evenement)
        try:
    ...
    @classmethod
    def annuler_reservation(cls,nouveau_status, rsv_id):
        sql = "UPDATE reservations SET status=%s WHERE id=%s"
        params = (nouveau_status, rsv_id)
        try:
